...
public:
    ...
    {   
        //变刚度
        for(int i=0; i<3; i++)  // 仅位置方向变刚度，姿态方向不变
        {
            double abs_f = std::abs(F_ext[i]);
            if(abs_f > force_threshold)
            {
                // 线性增加刚度，但不超过最大刚度
                double delta_K = stiffness_slope * (abs_f - force_threshold);
                
                // 一阶低通滤波平滑刚度变化（防止突变）
                double alpha = 0.5;
                K[i] = std::min(K[i] + alpha * delta_K, max_stiffness);
                //K[i] = alpha * new_K + (1-alpha) * K[i];
            }
        }
...
private:
    ...
    double x_d[6]={0.320000,0.235000,0};   // 位移
    double dx_d[6]={0,0,0,0,0,0};  // 速度

    double d_x,d_y,d_z;
    double offset_x=0.320000;
    double offset_y=0.235000;
    double offset_z=0;
    double distance;
    ...
    const double force_threshold = 50.0;      // 力阈值(N)，超过后刚度开始增加
    const double max_stiffness = 5000.0;      // 最大刚度(N/m)
    const double stiffness_slope = 50.0;      // 刚度增量斜率 (N/m)/N
};
