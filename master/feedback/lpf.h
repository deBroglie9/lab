...
#ifdef __cplusplus
extern "C" {
extern void butterworthLPF(BiquadFilter* filter, float cutoffFreq, float sampleRate);
extern void initFilter(BiquadFilter* filter);
extern float processSample(BiquadFilter* filter, float input);
extern float read_csv_float(const char* filename, int start_row, int col);
}
#endif
// // 计算巴特沃斯滤波器系数
