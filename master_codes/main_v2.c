/*****************************************************************************
 * 2024/12/18
 * 目前接入1个编码器 1个驱动器 代码目前可连接8个从站，1个编码器+7个驱动器，需要修改
 * DRIVER_NUMBER 
 * ENCODER_NUMBER 
 * DOMAIN_NUMBER 
 * 以及其他初始化函数的个数及序号
...
void cyclic_task()
{
    ...
    {   
        ...
        if(time_diff < RUNNING_TIME){
            ...
            float angle_all[1];
            ssi=EC_READ_U32(domain_pd[1] + off_bytes_ssi[0]);
            data_pointer[0]=ssi;
            memcpy(angle_all, data_pointer, sizeof(data_pointer));
            Target_Position=angle_all[0];
            printf("ssi=%f\n",Target_Position);
            ssi=EC_READ_U32(domain_pd[1] + off_bytes_ssi_2[0]);
            data_pointer[0]=ssi;
            memcpy(angle_all, data_pointer, sizeof(data_pointer));
            ...
            if(flag2==0 && Actual_Position!=0 && Target_Position!=0){
                if((abs(Target_Position)>360 || abs(Actual_Position)>360)){
                    Target_Position=0;
                    Actual_Position=0;
                }
                else
...
int main(int argc, char **argv)
{
    ...
    //     return -1;
    // }
        if (ecrt_domain_reg_pdo_entry_list(domain[1], domain8_regs)) 
    {
