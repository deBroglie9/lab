 ...
 * 用于四自由度双臂
 * 单\双臂柔顺实验测试
 * 修改宏定义的CONFIGURE_SINGLE CONFIGURE_DUAL可切换单双臂
 * 电机可以正常使能并进行运动
...
#define CONFIGURE_SINGLE 1
#define CONFIGURE_DUAL 0
// 任务周期（以 ns 为单位）
...
#define PERIODS_PER_SECOND_H   1000
...
double Ts=PERIODS_PER_SECOND_L/1000.0;
...
float sampleRate = PERIODS_PER_SECOND_H;    // 采样率
...
parameters_compliance admittance_params={.Md=1000, .Kd=3000, .Bd=5000}; //3 3 5
...
double loop_4DOF(double Px_r, double Py_r, double Pz_r, double cy_r,
    ...
    t4_a=t4_a+1;
    tp_a=tp_a+1;
    Actual_joint_Torque[0]=tr_a;
    Actual_joint_Torque[1]=tp_a;
    Actual_joint_Torque[2]=ty_a;
    Actual_joint_Torque[3]=t4_a;
...
double crlr, cplr, cylr, c4lr, crrr, cprr, cyrr, c4rr;

double Fe_d[3];
double c1l_a = 0, c2l_a = 0, c3l_a = 0, c4l_a = 0;
double t1l_a = 1, t2l_a = 1, t3l_a = 1;
double trl_a, tpl_a, tyl_a, t4l_a;
double crl_a, cpl_a, cyl_a;
double Pxl_a, Pyl_a, Pzl_a;

double c1r_a = 0, c2r_a = 0, c3r_a = 0, c4r_a = 0;
double t1r_a = 1, t2r_a = 1, t3r_a = 1;
double trr_a, tpr_a, tyr_a, t4r_a;
double crr_a, cpr_a, cyr_a;
double Pxr_a, Pyr_a, Pzr_a;

double w4l, wrl, wpl, wyl;
double dw4l, dwrl, dwpl, dwyl;

// 状态变量（需要在多次调用之间保持）
double statel_CR = 0, statel_CP = 0, statel_CY = 0, statel_CE = 0;
double statel_WR = 0, statel_WP = 0, statel_WY = 0, statel_WE = 0;
double statel_dWR = 0, statel_dWP = 0, statel_dWY = 0, statel_dWE = 0;
int initializedl = 0;

// 输出变量
double crl, cpl, cyl, cel, c1l, c2l, c3l, c4l;

// 假设的输入值 - 您需要根据实际情况提供这些值
double wrlr = 0, wplr = 0, wylr = 0, welr = 0;
double dwrlr = 0, dwplr = 0, dwylr = 0, dwelr = 0;
double Tl_joint_r = 0, Tl_joint_p = 0, Tl_joint_y = 0, Tl_joint_e = 0;

double w4r, wrr, wpr, wyr;
double dw4r, dwrr, dwpr, dwyr;

// 状态变量（需要在多次调用之间保持）
double stater_CR = 0, stater_CP = 0, stater_CY = 0, stater_CE = 0;
double stater_WR = 0, stater_WP = 0, stater_WY = 0, stater_WE = 0;
double stater_dWR = 0, stater_dWP = 0, stater_dWY = 0, stater_dWE = 0;
int initializedr = 0;

// 输出变量
double crr, cpr, cyr, cer, c1r, c2r, c3r, c4r;

// 假设的输入值 - 您需要根据实际情况提供这些值
double wrrr = 0, wprr = 0, wyrr = 0, werr = 0;
double dwrrr = 0, dwprr = 0, dwyrr = 0, dwerr = 0;
double Tr_joint_r = 0, Tr_joint_p = 0, Tr_joint_y = 0, Tr_joint_e = 0;

double Al[16];
double tyl_d, trl_d, tpl_d,
       t4l_d, Bl[4];
...
DelayFilter* delay_filter[8];
DerivativeFilter* derivative_filter[16];


void init_4DOF_dual(){
    delay_filter[trl_filter] = create_delay_filter();
    delay_filter[tpl_filter] = create_delay_filter();
    delay_filter[tyl_filter] = create_delay_filter();
    delay_filter[t4l_filter] = create_delay_filter();
    delay_filter[trr_filter] = create_delay_filter();
    delay_filter[tpr_filter] = create_delay_filter();
    delay_filter[tyr_filter] = create_delay_filter();
    delay_filter[t4r_filter] = create_delay_filter();

    derivative_filter[wl_c4r] = create_derivative_filter(1.0, Ts);
    derivative_filter[wl_crr] = create_derivative_filter(1.0, Ts);
    derivative_filter[wl_cpr] = create_derivative_filter(1.0, Ts);
    derivative_filter[wl_cyr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwl_c4r] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwl_crr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwl_cpr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwl_cyr] = create_derivative_filter(1.0, Ts);
    derivative_filter[wr_c4r] = create_derivative_filter(1.0, Ts);
    derivative_filter[wr_crr] = create_derivative_filter(1.0, Ts);
    derivative_filter[wr_cpr] = create_derivative_filter(1.0, Ts);
    derivative_filter[wr_cyr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwr_c4r] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwr_crr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwr_cpr] = create_derivative_filter(1.0, Ts);
    derivative_filter[dwr_cyr] = create_derivative_filter(1.0, Ts);
}


double loop_4DOF_dual(double crlr, double cplr, double cylr, double c4lr, double crrr, double cprr, double cyrr, double c4rr, 
            double c1l_a, double c2l_a, double c3l_a, double c4l_a, double c1r_a, double c2r_a, double c3r_a, double c4r_a,
           double t1l_a, double t2l_a, double t3l_a, double t4l_a, double t1r_a, double t2r_a, double t3r_a, double t4r_a, 
           double* cl_out1, double* cl_out2, double* cl_out3, double* cl_out4, double* cr_out1, double* cr_out2, double* cr_out3, double* cr_out4){
    
        //Jacobian_S test

    Jacobian_S(c1l_a, c2l_a, c3l_a, c4l_a, t1l_a,
                       t2l_a, t3l_a, &crl_a, &cpl_a, &cyl_a,
                       &Pxl_a, &Pyl_a, &Pzl_a, &trl_a,
                       &tpl_a, &tyl_a);

    Jacobian_S(c1r_a, c2r_a, c3r_a, c4r_a, t1r_a,
                       t2r_a, t3r_a, &crr_a, &cpr_a, &cyr_a,
                       &Pxr_a, &Pyr_a, &Pzr_a, &trr_a,
                       &tpr_a, &tyr_a);
    ...
    w4l = process_derivative_filter(derivative_filter[wl_c4r], c4lr);
    wrl = process_derivative_filter(derivative_filter[wl_crr], crlr);
    wpl = process_derivative_filter(derivative_filter[wl_cpr], cplr);
    wyl = process_derivative_filter(derivative_filter[wl_cyr], cylr);
    dw4l = process_derivative_filter(derivative_filter[dwl_c4r], w4l);
    dwrl = process_derivative_filter(derivative_filter[dwl_crr], wrl);
    dwpl = process_derivative_filter(derivative_filter[dwl_cpr], wpl);
    dwyl = process_derivative_filter(derivative_filter[dwl_cyr], wyl);
    w4r = process_derivative_filter(derivative_filter[wr_c4r], c4rr);
    wrr = process_derivative_filter(derivative_filter[wr_crr], crrr);
    wpr = process_derivative_filter(derivative_filter[wr_cpr], cprr);
    wyr = process_derivative_filter(derivative_filter[wr_cyr], cyrr);
    dw4r = process_derivative_filter(derivative_filter[dwr_c4r], w4r);
    dwrr = process_derivative_filter(derivative_filter[dwr_crr], wrr);
    dwpr = process_derivative_filter(derivative_filter[dwr_cpr], wpr);
    dwyr = process_derivative_filter(derivative_filter[dwr_cyr], wyr);

    //dynamics test
    Al[2]=1.0;
    Al[5]=1.0;
    Al[8]=1.0;
    Al[15]=((sin(cplr)*sin(cylr) + cos(cplr)*cos(cylr)*sin(crlr))*(sin(cplr)*sin(cylr) + cos(cplr)*cos(cylr)*sin(crlr)) + (cos(cplr)*sin(cylr) - cos(cylr)*sin(cplr)*sin(crlr))*(cos(cplr)*sin(cylr) - cos(cylr)*sin(cplr)*sin(crlr)) + cos(crlr)*cos(crlr)*cos(cylr)*cos(cylr));
    Ar[2]=1.0;
    Ar[5]=1.0;
    Ar[8]=1.0;
    Ar[15]=((sin(cprr)*sin(cyrr) + cos(cprr)*cos(cyrr)*sin(crrr))*(sin(cprr)*sin(cyrr) + cos(cprr)*cos(cyrr)*sin(crrr)) + (cos(cprr)*sin(cyrr) - cos(cyrr)*sin(cprr)*sin(crrr))*(cos(cprr)*sin(cyrr) - cos(cyrr)*sin(cprr)*sin(crrr)) + cos(crrr)*cos(crrr)*cos(cyrr)*cos(cyrr));
    
    // printf("input:cp: %f, cr: %f, cy: %f, c4: %f, wp: %f, wr: %f, wy: %f, w4: %f, dwp: %f, dwr: %f, dwy: %f, dw4: %f\n", cpr, crr, cyr, c4r, wp, wr, wy, w4, dwp, dwr, dwy, dw4);  
    Dynamics_4DOF(Fe_d, Al, cplr,
                   crlr, cylr, c4lr, wpl, wrl,
                   wyl, w4l, dwpl, dwrl, dwyl,
                   dw4l, &tyl_d, &trl_d, &tpl_d,
                   &t4l_d, Bl);
    Dynamics_4DOF(Fe_d, Ar, cprr,
                   crrr, cyrr, c4rr, wpr, wrr,
                   wyr, w4r, dwpr, dwrr, dwyr,
                   dw4r, &tyr_d, &trr_d, &tpr_d,
                   &t4r_d, Br);


    // if( trl_a < 3 + trl_d && trl_a > - 3 + trl_d ){
    //     trl_a = 0 + trl_d;
    // }
    // else{
    //     trl_a = trl_a;
    // }
    // if( tpl_a < 4 + tpl_d && tpl_a > - 4 + tpl_d ){
    //     tpl_a = 0 + tpl_d;
    // }
    // else{
    //     tpl_a = tpl_a;
    // }
    // if( tyl_a < 2.5 + tyl_d && tyl_a > - 2.5 + tyl_d ){
    //         tyl_a = 0 + tyl_d;
    //     }
    //     else{
    //         tyl_a = tyl_a;
    //     }
    // if( t4l_a < 3 + t4l_d && t4l_a > - 3 + t4l_d ){
    //         t4l_a = 0 + t4l_d;
    //     }
    //     else{
    //         t4l_a = t4l_a;
    //     }
    // if( trr_a < 3 + trr_d && trr_a > - 3 + trr_d ){
    //         trr_a = 0 + trr_d;
    //     }
    //     else{
    //         trr_a = trr_a;
    //     }
    // if( tpr_a < 4 + tpr_d && tpr_a > - 4 + tpr_d ){
    //         tpr_a = 0 + tpr_d;
    //     }
    //     else{
    //         tpr_a = tpr_a;
    //     }
    // if( tyr_a < 2.5 + tyr_d && tyr_a > - 2.5 + tyr_d ){
    //         tyr_a = 0 + tyr_d;
    //     }
    //     else{
    //         tyr_a = tyr_a;
    //     }
    // if( t4r_a < 3 + t4r_d && t4r_a > - 3 + t4r_d ){
    //         t4r_a = 0 + t4r_d;
    //     }
    //     else{
    //         t4r_a = t4r_a;
    ...
    // t4r_a=t4r_a+1;
    // tpr_a=tpr_a+1;
    Actual_joint_Torque[0]=trl_a;
    Actual_joint_Torque[1]=tpl_a;
    Actual_joint_Torque[2]=tyl_a;
    Actual_joint_Torque[3]=t4l_a;
    Actual_joint_Torque[4]=trr_a;
    Actual_joint_Torque[5]=tpr_a;
    Actual_joint_Torque[6]=tyr_a;
    Actual_joint_Torque[7]=t4r_a;
    printf("tyaw: %f, troll: %f, tpitch: %f, ta4: %f\n", tyl_d, trl_d, tpl_d, t4l_d);

    //filter test


    trl_a = process_delay_filter(delay_filter[trl_filter], trl_a);
    tpl_a = process_delay_filter(delay_filter[tpl_filter], tpl_a);
    tyl_a = process_delay_filter(delay_filter[tyl_filter], tyl_a);
    t4l_a = process_delay_filter(delay_filter[t4l_filter], t4l_a);
    trr_a = process_delay_filter(delay_filter[trr_filter], trr_a);
    tpr_a = process_delay_filter(delay_filter[tpr_filter], tpr_a);
    tyr_a = process_delay_filter(delay_filter[tyr_filter], tyr_a);
    t4r_a = process_delay_filter(delay_filter[t4r_filter], t4r_a);

    //admittance control torque output

    tl_joint[0] = trl_d - trl_a;
    tl_joint[1] = tpl_d - tpl_a;
    tl_joint[2] = tyl_d - tyl_a;
    tl_joint[3] = t4l_d - t4l_a;
    tr_joint[0] = trr_d - trr_a;
    tr_joint[1] = tpr_d - tpr_a;
    tr_joint[2] = tyr_d - tyr_a;
    tr_joint[3] = t4r_d - t4r_a;
    // printf("t_joint output: %f, %f, %f, %f\n", t_joint[0], t_joint[1], t_joint[2], t_joint[3]);

    jointController(
        crlr, cplr, cylr, c4lr,
        wrlr, wplr, wylr, welr,
        dwrlr, dwplr, dwylr, dwelr,
        tl_joint[0], tl_joint[1], tl_joint[2], tl_joint[3],
        &crl, &cpl, &cyl, &cel,
        &statel_CR, &statel_CP, &statel_CY, &statel_CE,
        &statel_WR, &statel_WP, &statel_WY, &statel_WE,
        &statel_dWR, &statel_dWP, &statel_dWY, &statel_dWE,
        &initializedl,
        admittance_params.Md, admittance_params.Kd, admittance_params.Bd, Ts);
    ...
    jointController(
        crrr, cprr, cyrr, c4rr,
        wrrr, wprr, wyrr, werr,
        dwrrr, dwprr, dwyrr, dwerr,
        tr_joint[0], tr_joint[1], tr_joint[2], tr_joint[3],
        &crr, &cpr, &cyr, &cer,
        &stater_CR, &stater_CP, &stater_CY, &stater_CE,
        &stater_WR, &stater_WP, &stater_WY, &stater_WE,
        &stater_dWR, &stater_dWP, &stater_dWY, &stater_dWE,
        &initializedr,
        admittance_params.Md, admittance_params.Kd, admittance_params.Bd, Ts);
    ...
    //IK_3DOF test
    IK_3DOF(crl, cpl, cyl, &c1l, &c2l, &c3l);
    c4l=cel;
    // printf("c1: %f, c2: %f, c3: %f, c4: %f\n", c1, c2, c3, c4);
    ...
    *cl_out4=c4l;
    ...
    IK_3DOF(crr, cpr, cyr, &c1r, &c2r, &c3r);
    c4r=cer;
    // printf("c1: %f, c2: %f, c3: %f, c4: %f\n", c1, c2, c3, c4);
...
// double Pxs_ad_r, Pys_ad_r, Pzs_ad_r;
// double Pxs_ad_l, Pys_ad_l, Pzs_ad_l;
...
// parameters_compliance admittance_params_2[4]; 


// double state_CR_2[4]={0,0,0,0}, state_CP_2[4]={0,0,0,0}, state_CY_2[4]={0,0,0,0}, state_CE_2[4]={0,0,0,0};
// double state_WR_2[4]={0,0,0,0}, state_WP_2[4]={0,0,0,0}, state_WY_2[4]={0,0,0,0}, state_WE_2[4]={0,0,0,0};
// double state_dWR_2[4]={0,0,0,0}, state_dWP_2[4]={0,0,0,0}, state_dWY_2[4]={0,0,0,0}, state_dWE_2[4]={0,0,0,0};
// int initialized_2[6]={0,0,0,0,0,0};

// double state_Fed_x[2] = {0,0}, state_Fed_y[2] = {0,0}, state_Fed_z[2] = {0,0};
// double state_FEd_x[2] = {0,0}, state_FEd_y[2] = {0,0}, state_FEd_z[2] = {0,0};
// double state_Ex_e1[2] = {0,0}, state_Ex_e2[2] = {0,0}, state_Ex_e3[2] = {0,0};
// double state_Ex_E1[2] = {0,0}, state_Ex_E2[2] = {0,0}, state_Ex_E3[2] = {0,0};

// double Pxs_ad_r, Pys_ad_r, Pzs_ad_r;
// double Px_ad_r, Py_ad_r, Pz_ad_r, cy_ad_r;
// double Pxs_ad_l, Pys_ad_l, Pzs_ad_l;
// double Px_ad_l, Py_ad_l, Pz_ad_l, cy_ad_l;
...
// double FE_r1[4], FE_r2[4], FE_r3[4];

// double FEx_r, FEy_r, FEz_r;
// double Fex_r, Fey_r, Fez_r;
// double FEx_l, FEy_l, FEz_l;
// double Fex_l, Fey_l, Fez_l;

// double Fed_r_1, Fed_r_2, Fed_r_3;
// double FEd_r_1, FEd_r_2, FEd_r_3;
// double Fed_l_1, Fed_l_2, Fed_l_3;
// double FEd_l_1, FEd_l_2, FEd_l_3;

// double loop_start_time;

// void init_4DOF_2(){
//     delay_filter[filter_Ex_e_r_1] = create_delay_filter();
//     delay_filter[filter_Ex_e_r_2] = create_delay_filter();
//     delay_filter[filter_Ex_e_r_3] = create_delay_filter();
//     delay_filter[filter_Ex_E_r_1] = create_delay_filter();
//     delay_filter[filter_Ex_E_r_2] = create_delay_filter();
//     delay_filter[filter_Ex_E_r_3] = create_delay_filter();
//     delay_filter[filter_Edx_e_r_1] = create_delay_filter();
//     delay_filter[filter_Edx_e_r_2] = create_delay_filter();
//     delay_filter[filter_Edx_e_r_3] = create_delay_filter();
//     delay_filter[filter_Ex_e_l_1] = create_delay_filter();
//     delay_filter[filter_Ex_e_l_2] = create_delay_filter();
//     delay_filter[filter_Ex_e_l_3] = create_delay_filter();
//     delay_filter[filter_Ex_E_l_1] = create_delay_filter();
//     delay_filter[filter_Ex_E_l_2] = create_delay_filter();
//     delay_filter[filter_Ex_E_l_3] = create_delay_filter();
//     delay_filter[filter_Edx_e_l_1] = create_delay_filter();
//     delay_filter[filter_Edx_e_l_2] = create_delay_filter();
//     delay_filter[filter_Edx_e_l_3] = create_delay_filter();

//     admittance_params_2[e_r].Md=1; admittance_params_2[e_r].Kd=5; admittance_params_2[e_r].Bd=4;
//     admittance_params_2[E_r].Md=1; admittance_params_2[E_r].Kd=5; admittance_params_2[E_r].Bd=4;
//     admittance_params_2[e_l].Md=1; admittance_params_2[e_l].Kd=5; admittance_params_2[e_l].Bd=4;
//     admittance_params_2[E_l].Md=1; admittance_params_2[E_l].Kd=5; admittance_params_2[E_l].Bd=4;
// }

// double loop_4DOF_2(double Pxd_r, double Pyd_r, double Pzd_r, double cyd_r,
//            double* c1_r, double* c2_r, double* c3_r, double* c4_r, double* cr_r, double* cp_r, double* cy_r,
//            double* c1_l, double* c2_l, double* c3_l, double* c4_l, double* cr_l, double* cp_l, double* cy_l){
//     //计时
//     if(!flag_loop){
//         gettimeofday(&tv2, NULL);
//         time_diff = (double)( tv2.tv_sec - tv1.tv_sec + (double)(tv2.tv_usec - tv1.tv_usec)/ 1000000);
//         loop_start_time = time_diff;
//         flag_loop=1;
//     }
//     double t;
//     gettimeofday(&tv2, NULL);
//     time_diff = (double)( tv2.tv_sec - tv1.tv_sec + (double)(tv2.tv_usec - tv1.tv_usec)/ 1000000);
//     t = time_diff - loop_start_time;


//     double Pxd_r, Pyd_r, Pzd_r, cyd_r;
//     double c4, cr, cp, cy;
//     //逆运动学
//     IK_1DOF(Pxd_r, Pyd_r, Pzd_r, cyd_r, &c4, &cr, &cp, &cy);

//     //接触点位置解算
//     double Pxe_r, Pye_r, Pze_r;
//     double PEx_r, PEy_r, PEz_r;
//     double R=86.5;
//     double L1=228.5;
//     double L2=343;
//     double Ls1=235;
...
//     Pxe_r=L1*cos(cp)*cos(cr) + R*cos(cp)*cos(cr);
//     Pye_r=L1*cos(cr)*sin(cp) + R*cos(cr)*sin(cp);
//     Pze_r=- L1*sin(cr) - R*sin(cr);

//     PEx_r=L1*cos(cp)*cos(cr) - Ls2*(sin(cp)*sin(cy) + cos(cp)*cos(cy)*sin(cr)) + R*cos(cp)*cos(cr) - Ls1*sin(c4)*(cos(cy)*sin(cp) - cos(cp)*sin(cr)*sin(cy)) + Ls1*cos(c4)*cos(cp)*cos(cr);
//     PEy_r=Ls2*(cos(cp)*sin(cy) - cos(cy)*sin(cp)*sin(cr)) + L1*cos(cr)*sin(cp) + R*cos(cr)*sin(cp) + Ls1*sin(c4)*(cos(cp)*cos(cy) + sin(cp)*sin(cr)*sin(cy)) + Ls1*cos(c4)*cos(cr)*sin(cp);
//     PEz_r=Ls1*cos(cr)*sin(c4)*sin(cy) - R*sin(cr) - Ls2*cos(cr)*cos(cy) - Ls1*cos(c4)*sin(cr) - L1*sin(cr);

//     //delay filter
//     Ex_e_r[0] = process_delay_filter(delay_filter[filter_Ex_e_r_1], Ex_e_r[0]);
//     Ex_E_r[0] = process_delay_filter(delay_filter[filter_Ex_E_r_1], Ex_E_r[0]);
//     Edx_e_r[0] = process_delay_filter(delay_filter[filter_Edx_e_r_1], Edx_e_r[0]);
//     Ex_e_l[0] = process_delay_filter(delay_filter[filter_Ex_e_l_1], Ex_e_l[0]);
//     Ex_E_l[0] = process_delay_filter(delay_filter[filter_Ex_E_l_1], Ex_E_l[0]);
//     Edx_e_l[0] = process_delay_filter(delay_filter[filter_Edx_e_l_1], Edx_e_l[0]);
//     Ex_e_r[1] = process_delay_filter(delay_filter[filter_Ex_e_r_2], Ex_e_r[1]);
//     Ex_E_r[1] = process_delay_filter(delay_filter[filter_Ex_E_r_2], Ex_E_r[1]);
//     Edx_e_r[1] = process_delay_filter(delay_filter[filter_Edx_e_r_2], Edx_e_r[1]);
//     Ex_e_l[1] = process_delay_filter(delay_filter[filter_Ex_e_l_2], Ex_e_l[1]);
//     Ex_E_l[1] = process_delay_filter(delay_filter[filter_Ex_E_l_2], Ex_E_l[1]);
//     Edx_e_l[1] = process_delay_filter(delay_filter[filter_Edx_e_l_2], Edx_e_l[1]);
//     Ex_e_r[2] = process_delay_filter(delay_filter[filter_Ex_e_r_3], Ex_e_r[2]);
//     Ex_E_r[2] = process_delay_filter(delay_filter[filter_Ex_E_r_3], Ex_E_r[2]);
//     Edx_e_r[2] = process_delay_filter(delay_filter[filter_Edx_e_r_3], Edx_e_r[2]);
//     Ex_e_l[2] = process_delay_filter(delay_filter[filter_Ex_e_l_3], Ex_e_l[2]);
//     Ex_E_l[2] = process_delay_filter(delay_filter[filter_Ex_E_l_3], Ex_E_l[2]);
//     Edx_e_l[2] = process_delay_filter(delay_filter[filter_Edx_e_l_3], Edx_e_l[2]);
    

//     //计算力
//     fcn(t,
//         Ex_e_r[0], Ex_e_r[1], Ex_e_r[2],
//         Ex_E_r[0], Ex_E_r[1], Ex_E_r[2],
//         Edx_e_r[0], Edx_e_r[1], Edx_e_r[2],
//         &Fed_r_1, &Fed_r_2, &Fed_r_3,
//         &FEd_r_1, &FEd_r_2, &FEd_r_3,
//         &state_Fed_x[0], &state_Fed_y[0], &state_Fed_z[0],
//         &state_FEd_x[0], &state_FEd_y[0], &state_FEd_z[0],
//         &state_Ex_e1[0], &state_Ex_e2[0], &state_Ex_e3[0],
//         &state_Ex_E1[0], &state_Ex_E2[0], &state_Ex_E3[0],
//         &initialized_2[4],
//         343, 5, 1, 5, 2.5,
//         FEx_r, FEy_r, FEz_r,
...
//     fcn(t,
//         Ex_e_l[0], Ex_e_l[1], Ex_e_l[2],
//         Ex_E_l[0], Ex_E_l[1], Ex_E_l[2],
//         Edx_e_l[0], Edx_e_l[1], Edx_e_l[2],
//         &Fed_l_1, &Fed_l_2, &Fed_l_3,
//         &FEd_l_1, &FEd_l_2, &FEd_l_3,
//         &state_Fed_x[1], &state_Fed_y[1], &state_Fed_z[1],
//         &state_FEd_x[1], &state_FEd_y[1], &state_FEd_z[1],
//         &state_Ex_e1[1], &state_Ex_e2[1], &state_Ex_e3[1],
//         &state_Ex_E1[1], &state_Ex_E2[1], &state_Ex_E3[1],
//         &initialized_2[5],
//         343, 5, 1, 5, 2.5,
//         FEx_l, FEy_l, FEz_l,
...
//     //
//     admittance_e(
//         FE_r1[e_r], FE_r2[e_r], FE_r3[e_r],
//         Pxd_r, Pyd_r, Pzd_r,
//         &Pxs_ad_r, &Pys_ad_r, &Pzs_ad_r,
//         &Ex_e_r[0], &Ex_e_r[1], &Ex_e_r[2],
//         &Edx_e_r[0], &Edx_e_r[1], &Edx_e_r[2],
//         &state_CR_2[e_r], &state_CP_2[e_r], &state_CY_2[e_r],
//         &state_WR_2[e_r], &state_WP_2[e_r], &state_WY_2[e_r],
//         &state_dWR_2[e_r], &state_dWP_2[e_r], &state_dWY_2[e_r],
//         &initialized_2[e_r],
//         admittance_params_2[e_r].Md, admittance_params_2[e_r].Kd, admittance_params_2[e_r].Bd, Ts);

//     admittance_E(
//         FE_r1[E_r], FE_r2[E_r], FE_r3[E_r],
//         Pxd_r, Pyd_r, Pzd_r, cyd_r,
//         &Px_ad_r, &Py_ad_r, &Pz_ad_r, &cy_ad_r,
//         &Ex_E_r[0], &Ex_E_r[1], &Ex_E_r[2],
//         &state_CR_2[E_r], &state_CP_2[E_r], &state_CY_2[E_r], &state_CE_2[E_r],
//         &state_WR_2[E_r], &state_WP_2[E_r], &state_WY_2[E_r], &state_WE_2[E_r],
//         &state_dWR_2[E_r], &state_dWP_2[E_r], &state_dWY_2[E_r], &state_dWE_2[E_r],
//         &initialized_2[E_r],
//         admittance_params_2[E_r].Md, admittance_params_2[E_r].Kd, admittance_params_2[E_r].Bd, Ts);

//     admittance_e(
//         FE_r1[e_l], FE_r2[e_l], FE_r3[e_l],
//         Pxd_r, Pyd_r, Pzd_r,
//         &Pxs_ad_l, &Pys_ad_l, &Pzs_ad_l,
//         &Ex_e_l[0], &Ex_e_l[1], &Ex_e_l[2],
//         &Edx_e_l[0], &Edx_e_l[1], &Edx_e_l[2],
//         &state_CR_2[e_l], &state_CP_2[e_l], &state_CY_2[e_l],
//         &state_WR_2[e_l], &state_WP_2[e_l], &state_WY_2[e_l],
//         &state_dWR_2[e_l], &state_dWP_2[e_l], &state_dWY_2[e_l],
//         &initialized_2[e_l],
//         admittance_params_2[e_l].Md, admittance_params_2[e_l].Kd, admittance_params_2[e_l].Bd, Ts);

//     admittance_E(
//         FE_r1[E_l], FE_r2[E_l], FE_r3[E_l],
//         Pxd_r, Pyd_r, Pzd_r, cyd_r,
//         &Px_ad_l, &Py_ad_l, &Pz_ad_l, &cy_ad_l,
//         &Ex_E_l[0], &Ex_E_l[1], &Ex_E_l[2],
//         &state_CR_2[E_l], &state_CP_2[E_l], &state_CY_2[E_l], &state_CE_2[E_l],
//         &state_WR_2[E_l], &state_WP_2[E_l], &state_WY_2[E_l], &state_WE_2[E_l],
//         &state_dWR_2[E_l], &state_dWP_2[E_l], &state_dWY_2[E_l], &state_dWE_2[E_l],
//         &initialized_2[E_l],
//         admittance_params_2[E_l].Md, admittance_params_2[E_l].Kd, admittance_params_2[E_l].Bd, Ts);


//     IK_1(Px_ad_r, Py_ad_r, Pz_ad_r, cy_ad_r, c4_r, cy_r);
//     IK_1(Px_ad_l, Py_ad_l, Pz_ad_l, cy_ad_l, c4_l, cy_l);
//     IK_shoulder(Pxs_ad_r, Pys_ad_r, Pzs_ad_r, cr_r, cp_r);
//     IK_shoulder(Pxs_ad_l, Pys_ad_l, Pzs_ad_l, cr_l, cp_l);
//     IK_3DOF(*cr_r, *cp_r, *cy_r, c1_r, c2_r, c3_r);
//     IK_3DOF(*cr_l, *cp_l, *cy_l, c1_l, c2_l, c3_l);
...
{   
    ...
    {   
        ...
        counter = FREQUENCY/PERIODS_PER_SECOND_H-1;
            ...
            switch (stage){
                ...
                case 1: counter_loop++;
                    // EC_WRITE_S32(domain_pd[13]+off_bytes_0x60ff[13],1000);
                        ...
                        if((Error[0]<ERROR)&&(Error[1]<ERROR)&&(Error[2]<ERROR)&&(Error[3]<ERROR)&&(Error[4]<ERROR)&&(Error[5]<ERROR)&&(Error[6]<ERROR)&&(Error[7]<ERROR)&&(counter_loop/PERIODS_PER_SECOND_H>5)){
                ...
                case 2: 
                        ...
                        //        ,values[7*counter_loop+3],values[7*counter_loop+4],values[7*counter_loop+5],values[7*counter_loop+6]);
                                ...
                                //}
                                    ...
                                    //     Target_Position[7]=angle[counter_loop];
                                    // }
                                    // if (counter_loop<26400){
                                    //     Target_Position[3]=angle[counter_loop];
                                    //     Target_Position[7]=angle[counter_loop];
                                    // }
                                    // else{
                                    //     Target_Position[3]=angle[26400];
                                    //     Target_Position[7]=angle[26400];
                                    // }
                                    ...
                                    // {   
                                    // //torque_trans();
                                    // counter2 = PERIODS_PER_SECOND_H/PERIODS_PER_SECOND_L-1;
                                    ...
                                    // printf("target: ");
                                    // for(counter_slave=0;counter_slave<DRIVER_NUMBER;counter_slave++){
                                    //     Target_Position[counter_slave]=180/3.1415926536*values[8*counter_loop+counter_slave];
                                    //     printf("%f ",Target_Position[counter_slave]);
                                    // }
                                    // printf("\n");

                                    // if(counter_loop>5000&&counter_loop<8000){
                                    //     for(counter_slave=0;counter_slave<DRIVER_NUMBER;counter_slave++){
                                    //         Position_Kp[counter_slave]=1;
                                    //     }
                                    //     Target_Position[0]=30.7397;
                                    //     Target_Position[1]=26.3068;
                                    //     Target_Position[2]=13.8867;
                                    //     Target_Position[3]=1;
                                    //     Target_Position[4]=30.7397;
                                    //     Target_Position[5]=26.3068;
                                    //     Target_Position[6]=13.8867;
                                    //     Target_Position[7]=1;
                                    // }
                                    // else if(counter_loop>8000){
                                    //     for(counter_slave=0;counter_slave<DRIVER_NUMBER;counter_slave++){
                                    //         Position_Kp[counter_slave]=1;
                                    //     }
                                    //     Target_Position[0]=30.7397;
                                    ...
                                    //     cplr=0;
                                    //     cylr=0;
                                    //     c4lr=5*3.1415926535/180;
                                    //     crrr=0;
                                    //     cprr=0;
                                    //     cyrr=0;
                                    //     c4rr=5*3.1415926535/180;
                                    // }
                                    // else if(counter_loop<6000){
                                    //     crlr=-10*3.1415926/180;
                                    //     cplr=10*3.1415926/180;
                                    //     cylr=0;
                                    //     c4lr=105*3.1415926535/180;
                                    //     crrr=-10*3.1415926/180;
                                    //     cprr=10*3.1415926/180;
                                    //     cyrr=0;
                                    //     c4rr=105*3.1415926535/180;
                                    // }
                                    // else if(counter_loop<11000){
                                    //     crlr=-5*3.1415926/180;
                                    //     cplr=10*3.1415926/180;
                                    //     cylr=-5*3.1415926/180;
                                    //     c4lr=105*3.1415926535/180;
                                    //     crrr=-5*3.1415926/180;
                                    //     cprr=10*3.1415926/180;
                                    //     cyrr=0;
                                    //     c4rr=105*3.1415926535/180;
                                    // }
                                    // else{
                                    //     crlr=-5*3.1415926/180;
                                    //     cplr=20*3.1415926/180;
                                    //     cylr=-5*3.1415926/180;
                                    //     c4lr=100*3.1415926535/180;
                                    //     crrr=-5*3.1415926/180;
                                    //     cprr=20*3.1415926/180;
                                    //     cyrr=0;
                                    //     c4rr=100*3.1415926535/180;
                                    // }
                                    double dcr=4*3.1415926/180;
                                    double dcy=6*3.1415926/180;
                                    double dc4=10*3.1415926/180;
                                    if (counter_loop<1000){
                                        crlr=0;
                                        cplr=0;
                                        cylr=0;
                                    ...
                                    loop_4DOF_dual(crlr, cplr, cylr, c4lr, crrr, cprr, cyrr, c4rr, 
                                    Actual_Position[0]*3.1415926535/180, Actual_Position[1]*3.1415926535/180, Actual_Position[2]*3.1415926535/180, Actual_Position[3]*3.1415926535/180, Actual_Position[4]*3.1415926535/180, Actual_Position[5]*3.1415926535/180, Actual_Position[6]*3.1415926535/180, Actual_Position[7]*3.1415926535/180,
                                    Actual_Torque[0], Actual_Torque[1], Actual_Torque[2], Actual_Torque[3], Actual_Torque[4], Actual_Torque[5], Actual_Torque[6], Actual_Torque[7],
                                    &cl[0], &cl[1], &cl[2], &cl[3], &cr[0], &cr[1], &cr[2], &cr[3]);

                                    // if(counter_loop>=5000&counter_loop<15000){
                                    //     cl[0]=0.648;
                                    //     cl[1]=0.719;
                                    //     cl[2]=0.488;
                                    //     cl[3]=0.349;
                                    ...
                                    //     cr[3]=0.349;
                                    // }
                                    printf("cl: [%f, %f, %f, %f]\n", cl[0]*180/3.1415926536, cl[1]*180/3.1415926536, cl[2]*180/3.1415926536, cl[3]*180/3.1415926536);
                                    printf("cr: [%f, %f, %f, %f]\n", cr[0]*180/3.1415926536, cr[1]*180/3.1415926536, cr[2]*180/3.1415926536, cr[3]*180/3.1415926536);

                                    Target_Position[0]=cl[0]*180/3.1415926536;
                                    Target_Position[1]=cl[1]*180/3.1415926536;
                                    Target_Position[2]=cl[2]*180/3.1415926536;
                                    Target_Position[3]=cl[3]*180/3.1415926536;
                                    Target_Position[4]=cr[0]*180/3.1415926536;
                                    Target_Position[5]=cr[1]*180/3.1415926536;
                                    Target_Position[6]=cr[2]*180/3.1415926536;
                                    Target_Position[7]=cr[3]*180/3.1415926536;
                                    }
        ...
        //udp通信
        char sendBuf[1024];
        sprintf(sendBuf, "d:%f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f,  %f,%f,%f,%f,%f,%f,%f,%f\n",
            ...
            cl[0],cl[1],cl[2],cl[3],cr[0],cr[1],cr[2],cr[3]);//64-71
    ...
    for(i=0;i<48000;i++){
        if(i<2400){
            ...
            if(i<3600){
                ...
                angle[i]=55-35*cos(3.1415926536*(i-6000)/1200);
            }
            else{
                angle[i]=angle[i-6000];
            }
        }
