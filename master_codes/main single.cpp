 ...
 * 2025/9/16
 * 电机可以正常使能并进行运动
 * 目前接入0个编码器 1个驱动器 代码目前可连接16个从站，2个编码器+14个驱动器，需要修改 DRIVER_NUMBER ENCODER_NUMBER DOMAIN_NUMBER
...
//控制部分变量
//wrist  -10,10
static float Target_Position[DRIVER_NUMBER]={0};
...
static float max_velocity[DRIVER_NUMBER]={0.9*MAX_VELOCITY_1};
...
static float constant_Kp[DRIVER_NUMBER]={40000*19.36/360};
...
static float Position_Kp[DRIVER_NUMBER]={1};
    ...
    {   
        ...
        if((time_diff < RUNNING_TIME)&&(flag_init_all==1)&&(flag_stop_all!=1)){
            ...
            case 2: counter_loop++; 
                    ...
                    for(counter_slave=0;counter_slave<DRIVER_NUMBER;counter_slave++){
                        Position_Kp[counter_slave]=0.5;
                        Target_Position[counter_slave]=0;
                        tra=EC_READ_S16(domain_pd[counter_slave] + off_bytes_0x6077[counter_slave]);
                        torque=((float)tra)/1000.0*40.0;
                        // printf("error code:%d\n",tra);
                        // printf("error code:%f\n",(((float)tra)/1000.0*40.0));
                        printf("test0");
                        input = torque; // 读取current.csv第28行第4列（D28）
                        // 处理样本
                        ...
                        // 写入CSV文件
                        // fprintf(csvFile, "%d,%.6f,%.6f,%.6f\n", counter_loop, (float)counter_loop/1000, input, output);
                        printf("test2");
                        // torque=(int16_t)strtoul(ssi, NULL, 2);
                        // printf("torque:%d\n",torque);
                        // torque=(int)ssi;
                        // printf("torque:%d\n",torque);
                        // Actual_Position[counter_slave]=Actual_Position[counter_slave]/constant_Kp[counter_slave];
...
int main(int argc, char **argv)
{
    ...
    csvFile = fopen("filter_data.csv", "w");
    if (csvFile == NULL) {
        printf("无法创建CSV文件!\n");
        return 1;
    }
