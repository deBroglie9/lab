 ...
 * 2025/5/8
 * 电机可以正常使能并进行运动
 * 目前接入1个编码器 14个驱动器 代码目前可连接15个从站，1个编码器+14个驱动器，需要修改 DRIVER_NUMBER ENCODER_NUMBER DOMAIN_NUMBER 
 * 设置reading_ssi和control_pid函数
 * 以及其他初始化函数的个数及序号
 * 加入逆运动学求解函数，读入X,Y,Z和R(3*3逐行读入)
 * 给定了一组轨迹，可可读入并执行（未调试），注意修改轨迹时修改对应csv的数组大小 NUMBER
...
static ec_slave_config_t *sc9;
static ec_slave_config_state_t sc9_ana_in_state = {};

static ec_slave_config_t *sc10;
static ec_slave_config_state_t sc10_ana_in_state = {};

static ec_slave_config_t *sc11;
static ec_slave_config_state_t sc11_ana_in_state = {};

static ec_slave_config_t *sc12;
static ec_slave_config_state_t sc12_ana_in_state = {};

static ec_slave_config_t *sc13;
static ec_slave_config_state_t sc13_ana_in_state = {};

static ec_slave_config_t *sc14;
static ec_slave_config_state_t sc14_ana_in_state = {};

static ec_slave_config_t *sc15;
static ec_slave_config_state_t sc15_ana_in_state = {};

static ec_slave_config_t *sc16;
static ec_slave_config_state_t sc16_ana_in_state = {};
...
const static ec_pdo_entry_reg_t domain16_regs[] = {
    // {Alias, Position, Vendor ID, Product Code, PDO Index, -Subindex, Offset}
    {BusCouplerPos15,ENCODERICE, 0x7020, 0x01, &off_bytes_cm_1[1], &off_bits_cm_1[1]},
    {BusCouplerPos15,ENCODERICE, 0x7020, 0x02, &off_bytes_cm_2[1], &off_bits_cm_2[1]},
    {BusCouplerPos15,ENCODERICE, 0x7030, 0x01, &off_bytes_target_angle[1], &off_bits_target_angle[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x01, &off_bytes_ssi[1], &off_bits_ssi[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x02, &off_bytes_ssi_2[1], &off_bits_ssi_2[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x03, &off_bytes_ssi_3[1], &off_bits_ssi_3[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x04, &off_bytes_ssi_4[1], &off_bits_ssi_4[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x05, &off_bytes_ssi_5[1], &off_bits_ssi_5[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x06, &off_bytes_ssi_6[1], &off_bits_ssi_6[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x07, &off_bytes_ssi_7[1], &off_bits_ssi_7[1]},
    {BusCouplerPos15,ENCODERICE, 0x6000, 0x08, &off_bytes_ssi_8[1], &off_bits_ssi_8[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x01, &off_bytes_ain_1[1], &off_bits_ain_1[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x02, &off_bytes_ain_2[1], &off_bits_ain_2[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x03, &off_bytes_ain_3[1], &off_bits_ain_3[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x04, &off_bytes_ain_4[1], &off_bits_ain_4[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x05, &off_bytes_ain_5[1], &off_bits_ain_5[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x06, &off_bytes_ain_6[1], &off_bits_ain_6[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x07, &off_bytes_ain_7[1], &off_bits_ain_7[1]},
    {BusCouplerPos15,ENCODERICE, 0x6010, 0x08, &off_bytes_ain_8[1], &off_bits_ain_8[1]},
    {}
};
...
void check_slave9_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc9, &s);

    if (s.al_state != sc9_ana_in_state.al_state)
        printf("AnaIn9: State 0x%02X.\n", s.al_state);

    if (s.online != sc9_ana_in_state.online)
        printf("AnaIn9: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc9_ana_in_state.operational)
        printf("AnaIn9: %soperational.\n",
            s.operational ? "" : "Not ");

    sc9_ana_in_state = s;
}

void check_slave10_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc10, &s);

    if (s.al_state != sc10_ana_in_state.al_state)
        printf("AnaIn10: State 0x%02X.\n", s.al_state);

    if (s.online != sc10_ana_in_state.online)
        printf("AnaIn10: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc10_ana_in_state.operational)
        printf("AnaIn10: %soperational.\n",
            s.operational ? "" : "Not ");

    sc10_ana_in_state = s;
}

void check_slave11_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc11, &s);

    if (s.al_state != sc11_ana_in_state.al_state)
        printf("AnaIn11: State 0x%02X.\n", s.al_state);

    if (s.online != sc11_ana_in_state.online)
        printf("AnaIn11: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc11_ana_in_state.operational)
        printf("AnaIn11: %soperational.\n",
            s.operational ? "" : "Not ");

    sc11_ana_in_state = s;
}

void check_slave12_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc12, &s);

    if (s.al_state != sc12_ana_in_state.al_state)
        printf("AnaIn12: State 0x%02X.\n", s.al_state);

    if (s.online != sc12_ana_in_state.online)
        printf("AnaIn12: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc12_ana_in_state.operational)
        printf("AnaIn12: %soperational.\n",
            s.operational ? "" : "Not ");

    sc12_ana_in_state = s;
}

void check_slave13_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc13, &s);

    if (s.al_state != sc13_ana_in_state.al_state)
        printf("AnaIn13: State 0x%02X.\n", s.al_state);

    if (s.online != sc13_ana_in_state.online)
        printf("AnaIn13: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc13_ana_in_state.operational)
        printf("AnaIn13: %soperational.\n",
            s.operational ? "" : "Not ");

    sc13_ana_in_state = s;
}

void check_slave14_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc14, &s);

    if (s.al_state != sc14_ana_in_state.al_state)
        printf("AnaIn14: State 0x%02X.\n", s.al_state);

    if (s.online != sc14_ana_in_state.online)
        printf("AnaIn14: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc14_ana_in_state.operational)
        printf("AnaIn14: %soperational.\n",
            s.operational ? "" : "Not ");

    sc14_ana_in_state = s;
}

void check_slave15_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc15, &s);

    if (s.al_state != sc15_ana_in_state.al_state)
        printf("AnaIn15: State 0x%02X.\n", s.al_state);

    if (s.online != sc15_ana_in_state.online)
        printf("AnaIn15: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc15_ana_in_state.operational)
        printf("AnaIn15: %soperational.\n",
            s.operational ? "" : "Not ");

    sc15_ana_in_state = s;
}

void check_slave16_config_states(void)
{
    // printf("check_slave_config_states\n");
    ec_slave_config_state_t s;

    ecrt_slave_config_state(sc16, &s);

    if (s.al_state != sc16_ana_in_state.al_state)
        printf("AnaIn15: State 0x%02X.\n", s.al_state);

    if (s.online != sc16_ana_in_state.online)
        printf("AnaIn16: %s.\n", s.online ? "online" : "offline");
    if (s.operational != sc16_ana_in_state.operational)
        printf("AnaIn16: %soperational.\n",
            s.operational ? "" : "Not ");
        ...
        master, BusCouplerPos16, ENCODERICE)))
{
    ...
    if (ecrt_slave_config_pdos(sc8, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc9, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc10, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc11, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc12, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc13, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc14, EC_END, slave_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc15, EC_END, slave_0_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc16, EC_END, slave_0_syncs))
    {
        fprintf(stderr, "Failed to configure PDOs.\n");
        return -1;
    }
...
void cyclic_task()
{
    ...
    {   
        ...
        check_slave9_config_states();
        check_slave10_config_states();
        check_slave11_config_states();
        check_slave12_config_states();
        check_slave13_config_states();
        check_slave14_config_states();
        check_slave15_config_states();
        check_slave16_config_states();
        // main_IK_7DOF();
        ...
        //电机使能
        else if((time_diff < RUNNING_TIME) && !(flag_init[0]&flag_init[1]&flag_init[2]&flag_init[3]&flag_init[4]&flag_init[5]&flag_init[6]&flag_init[7]&flag_init[8]&flag_init[9]&flag_init[10]&flag_init[11]&flag_init[12]&flag_init[13])==1){
            // driver_enable(6);
            ...
            {   if(!flag_init[counter_slave]){
                ...
                printf("%d",flag_init[counter_slave]);
            }
            printf("\n");
        }
        //控制部分
...
int main(int argc, char **argv)
{
    ...
    // 打开文件
    file = fopen("circleq_start.csv", "r");
    if (file == NULL) {
